...
def top_n_largest_chunked(path, row_filter, sort_col, n, select_cols):
    """Čita parquet u chunk-ovima i vraća globalnih top N redova po sort_col."""
    try:
...
def handler(event, context):
    ...
    # 1. Dnevni broj HN objava po tipu — partition filter + chunked da ne pukne RAM
    try:
        ...
        for chunk in chunks:
            ...
            for post_type, cnt in hn_chunk["post_type"].value_counts().items():
                daily_counts[post_type] = daily_counts.get(post_type, 0) + cnt
    except Exception:
        daily_counts = {}
    ...
    # 2. Dnevni broj HN korisnika — users tabela je mala, čitamo sve
    try:
    ...
    # 3. Top 10 HN korisnika sa najvećim karma — users tabela je mala
    if not hn_users.empty:
        top_high = (
    ...
    # 4. Top 10 HN korisnika sa najmanjim karma — users tabela je mala
    if not hn_users.empty:
        bottom_karma = (
            hn_users[hn_users["karma_score"].notna()]
    ...
    # 5. Top 10 HN job objava po score-u — posts tabela je velika, čitamo u chunk-ovima
    ...
    # 6. Top 10 HN story objava po score-u — posts tabela je velika, čitamo u chunk-ovima
